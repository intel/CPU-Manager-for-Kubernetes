...
**What will `prepare_release.py` do :**
 ...
 - check whether there is no `kcm-release-v0.6.0` branch neither locally nor remotely
 - get previous version string from `Makefile` (`version=v0.6.0`) and check

If all above checks pass, script will:
 - create local branch `kcm-release-v0.6.0`
 - replace old release string (`v0.6.0`) with new one (`v0.6.0`) in all repo files
 - commit changes with message `KCM release - version v0.6.0.`
...
- check latest commit message for `KCM release - version v0.6.0.` string
- `v0.6.0` will become tag value
- generate change log
