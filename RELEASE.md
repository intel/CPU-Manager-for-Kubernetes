...
**What will `prepare_release.py` do :**
 ...
 - check whether there is no `cmk-release-v1.0.0` branch neither locally nor remotely
 - get previous version string from `Makefile` (`version=v1.0.0`) and check

If all above checks pass, script will:
 - create local branch `cmk-release-v1.0.0`
 - replace old release string (`v1.0.0`) with new one (`v1.0.0`) in all repo files
 - commit changes with message `CMK release - version v1.0.0.`
...
- check latest commit message for `CMK release - version v1.0.0.` string
- `v1.0.0` will become tag value
- generate change log
