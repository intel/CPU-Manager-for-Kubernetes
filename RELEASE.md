...
**What will `prepare_release.py` do :**
 ...
 - check whether there is no `kcm-release-v0.6.1` branch neither locally nor remotely
 - get previous version string from `Makefile` (`version=v0.6.1`) and check

If all above checks pass, script will:
 - create local branch `kcm-release-v0.6.1`
 - replace old release string (`v0.6.1`) with new one (`v0.6.1`) in all repo files
 - commit changes with message `KCM release - version v0.6.1.`
...
- check latest commit message for `KCM release - version v0.6.1.` string
- `v0.6.1` will become tag value
- generate change log
